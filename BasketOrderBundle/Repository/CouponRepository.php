...
class CouponRepository extends ServiceEntityRepository
{
    ...
    public function findByShK($couponShK): ?Coupon
    {
        $coupon = $this->createQueryBuilder('c')
            ->andWhere('c.codeDetailSap = :couponShK')
            ->setParameter('couponShK', $couponShK)
            ->andWhere('CURRENT_DATE() BETWEEN c.datBeginSap AND c.datEndSap')
            ->orderBy('c.id', 'DESC')
            ->setMaxResults(1)
            ->getQuery()
            ->getResult()
